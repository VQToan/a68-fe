...
export const fetchBotTemplates = createAsyncThunk(
  ...
  async (
    ...
    try {
      const templates = await botTemplateService.getBotTemplates(
        keyword,
        skip,
        limit
      );
      ...
      return rejectWithValue(
        err.response?.data?.detail || "Failed to fetch bot templates"
      );
    }
  }
);
...
export const fetchActiveBotTemplates = createAsyncThunk(
  "botTemplate/fetchActiveTemplates",
  async (_, { rejectWithValue }) => {
    try {
      const templates = await botTemplateService.getActiveBotTemplates();
      return templates;
    } catch (error: unknown) {
      const err = error as Error & {
        response?: { data?: { detail?: string } };
      ...
      return rejectWithValue(
        err.response?.data?.detail || "Failed to fetch active bot templates"
      );
    }
  }
);
...
export const fetchBotTemplateById = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        err.response?.data?.detail || "Failed to fetch bot template"
      );
    }
  }
);
...
export const createBotTemplate = createAsyncThunk(
  ...
  async (templateData: BotTemplateCreate, { rejectWithValue }) => {
    try {
      const newTemplate = await botTemplateService.createBotTemplate(
        templateData
      );
      return newTemplate;
    } catch (error: unknown) {
      ...
      return rejectWithValue(
        err.response?.data?.detail || "Failed to create bot template"
      );
    }
  }
);
...
export const updateBotTemplate = createAsyncThunk(
  ...
  async (
    { id, updates }: { id: string; updates: BotTemplateUpdate },
    { rejectWithValue }
  ) => {
    try {
      const updatedTemplate = await botTemplateService.updateBotTemplate(
        id,
        updates
      );
      ...
      return rejectWithValue(
        err.response?.data?.detail || "Failed to update bot template"
      );
    }
  }
);
...
export const deleteBotTemplate = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        err.response?.data?.detail || "Failed to delete bot template"
      );
    }
  }
);
...
const botTemplateSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        fetchBotTemplates.fulfilled,
        (state, action: PayloadAction<BotTemplate[]>) => {
      ...
      // Fetch active bot templates
      .addCase(fetchActiveBotTemplates.pending, (state) => {
        state.isLoading = true;
        state.error = null;
      })
      .addCase(
        fetchActiveBotTemplates.fulfilled,
        (state, action: PayloadAction<BotTemplate[]>) => {
          state.isLoading = false;
          state.activeTemplates = action.payload;
        }
      )
      .addCase(fetchActiveBotTemplates.rejected, (state, action) => {
        state.isLoading = false;
        state.error = action.payload as string;
      })
      ...
      .addCase(
        fetchBotTemplateById.fulfilled,
        (state, action: PayloadAction<BotTemplate>) => {
          state.isLoading = false;
